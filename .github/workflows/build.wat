  ...
  create:
    ...
    steps:
      - name: Create release
        ...
        run: |
          tmpfile=$(mktemp)
          echo '{"tag_name":"${{ needs.check.outputs.FRIDA_VERSION }}","name":"${{ needs.check.outputs.FRIDA_VERSION }}","body":"${{ needs.check.outputs.FRIDA_VERSION }}","draft":true}' | curl -fs -H "Authorization: token ${GITHUB_TOKEN}" -H "Content-Type: application/json" -d@- -o "${tmpfile}" "https://api.github.com/repos/${GITHUB_REPOSITORY}/releases"
    ...
    steps:
      ...
      - name: NDK 25c setup
        run: |
          ...
          rm -rf "${ANDROID_NDK%/*}/25.2.9519653"
          mv -v android-ndk-r25c "${ANDROID_NDK%/*}/25.2.9519653"
      ...
      - name: Install dependencies
        run: |
          [ $(id -u) -eq 0 ] || SUDO=sudo
          ${SUDO} apt-get update && DEBIAN_FRONTEND=noninteractive ${SUDO} apt-get install -y build-essential ninja-build gcc-multilib g++-multilib lib32stdc++-9-dev flex bison libc6-dev libc6-dev-i386
          ...
          if [ "${_CARCH%64}" = "${_CARCH}" ]; then
            ...
            patch -Rp1 <${{ github.workspace }}/pyconfig.h-i686-x86_64.patch
            popd
          fi

      - name: Clone frida-java-bridge
        run: |
      ...
      - name: Build Frida
        run: |
          ...
          set -e
          for k in strongR-frida florida; do
            for i in "../${k}/"*; do
              pushd "subprojects/${i##*/}"
          ...
          ./configure --prefix="${PWD}/build" --host="${{ matrix.os }}-${{ matrix.arch }}" --enable-frida-tools --enable-gadget --enable-server --enable-portal --enable-gadget -- -Dfrida-gum:devkits=glib
          make
          popd
      ...
      - name: Ship artifacts
        ...
        run: |
          FRIDA_VERSION="${{ needs.check.outputs.FRIDA_VERSION }}"
          gh_release_put(){ curl -fs -H "Authorization: token ${GITHUB_TOKEN}" -H "Content-Type: application/octet-stream" -o /dev/null --data-binary "@${1}" "https://uploads.github.com/repos/${GITHUB_REPOSITORY}/releases/assets?name=${2}" }
          export XZ_OPT="-9e -T0" ZSTD_NBTHREADS=0 ZSTD_CLEVEL=19
          ZSTD_OPT="-${ZSTD_CLEVEL:-22} --ultra -T${ZSTD_NBTHREADS:-0}" LZIP_OPT="-n$(nproc) -9" PIGZ="-p$(nproc) -11"
          compr="xz ${XZ_OPT}:xz"
          pushd frida
