    ...
    steps:
      ...
      - name: Apply patch to generate-runtime.py
        run: |
          ...
          # Применяем патч для generate-runtime.py
          patch -p1 <<EOF
      ...
      - name: Build Frida
        run: |
          ...
          for k in strongR-frida florida; do  # swe(aty)-russian capacity to coherently read may have been vastly overestimated
            for i in "../${k}/"*; do
              pushd "subprojects/${i##*/}"
